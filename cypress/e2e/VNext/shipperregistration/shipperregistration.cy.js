...
describe.only('The registrartion of shipper company', () => {
    it('Go through Registration Process', ()=>{
        ...
        cy.get('.form-control').eq(0).type('Track and Trace Service Inc')
        cy.get('.form-control').eq(1).type('24895 Long Valley Rd')
        cy.get('.form-control').eq(3).type('Hidden Hills')
        cy.get('.form-select').eq(0).select('CA',{force:true}).should('have.value', '3')
        cy.get('.form-control').eq(4).type('913023+{enter}')
        ...
        //Adding Contact 
             cy.contains('Add Contact').click()
              ...
              cy.get('input[type="text"]').eq(1).type('Testov')
              cy.get('input[type="text"]').eq(2).type('4545454585')
              cy.get('input[type="email"]').type('sehgghv.hauke@haulerhub.com')
